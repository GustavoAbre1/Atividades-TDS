...
def test_processar_produto():
    # Arrange
    product1 = Product(777, "Hamburger", 20, 5)
    product_repository = ProductRepository()
    product_repository.append_product(product1)

    order_product1 = OrderProduct()
    order_product1.add_product(product1, 2)

    # Act
  

    # Assert
    assert order_product1.product.id == 777
    assert order_product1.quantity == 3
    assert order_product1.value_item == 200

def test_baixar_produto():
    # Arrange
    product1 = Product(333, "Hamburger", 50, 35)
